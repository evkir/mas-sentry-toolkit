...
def sniff(broker, port, topic, duration, output):
    ...
    if output:
        data = [{"topic": m.topic, "payload": m.payload_str(), "qos": m.qos, "size": m.payload_size()} for m in msgs]
...
@click.option("--broker", default="127.0.0.1", show_default=True)
@click.option("--port", default=1883, show_default=True)
@click.option("--protocol", default="mqtt", type=click.Choice(["mqtt", "amqp"]), show_default=True)
...
def audit(broker, port, protocol, output, full):
    ...
    report = MASAuditReport(meta=ReportMeta(session_id=session_id, target=broker, protocol=protocol))
    if protocol == "mqtt":
        MQTTBrokerFingerprinter(broker, port).fingerprint()
