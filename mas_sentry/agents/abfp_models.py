...
class BehavioralBaseline:
    ...
    def deviation_score(self, fp: "AgentFingerprint") -> float:
        ...
        if self.expected_interval_ms > 0:
            timing_dev = abs(fp.timing.mean_interval_ms - self.expected_interval_ms) / self.expected_interval_ms
            score += min(timing_dev * 30.0, 30.0)

        # Payload size deviation
        if self.expected_payload_size > 0:
            payload_dev = abs(fp.payload.mean_size_bytes - self.expected_payload_size) / self.expected_payload_size
            score += min(payload_dev * 20.0, 20.0)

        # Entropy deviation
        if self.expected_entropy > 0:
            entropy_dev = abs(fp.payload.entropy_score - self.expected_entropy) / max(self.expected_entropy, 0.01)
