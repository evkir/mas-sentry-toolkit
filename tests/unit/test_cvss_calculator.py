...
from mas_sentry.threat_modeling.cvss_calculator import (
    CVSSVector, calculate_cvss, MQTT_ANON_ACCESS, MQTT_RETAINED_POISON
)
...
class TestCVSSCalculator:
    ...
    def test_local_access_lower_than_network(self):
        net = CVSSVector(attack_vector="N", confidentiality="H",
                         integrity="H", availability="H")
        local = CVSSVector(attack_vector="L", confidentiality="H",
                           integrity="H", availability="H")
        assert calculate_cvss(net) > calculate_cvss(local)

    def test_high_complexity_lowers_score(self):
        low = CVSSVector(attack_complexity="L", confidentiality="H",
                         integrity="H", availability="H")
        high = CVSSVector(attack_complexity="H", confidentiality="H",
                          integrity="H", availability="H")
    ...
    def test_score_bounded_0_to_10(self):
        v = CVSSVector(attack_vector="N", confidentiality="H",
                       integrity="H", availability="H")
        score = calculate_cvss(v)
