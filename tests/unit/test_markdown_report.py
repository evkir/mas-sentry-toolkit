...
def make_report():
    r = MASAuditReport(meta=ReportMeta(
        session_id="md-test", target="127.0.0.1", protocol="mqtt"))
    r.add_finding("Anon Access", "CRITICAL",
                  "Broker open", remediation="Add auth")
    r.abfp_fingerprints = [{
        "agent_id": "test_agent", "message_count": 30,
        "anomaly_score": 5.0, "threat_flags": []
    }]
    return r
