...
skip_no_broker = pytest.mark.skipif(
    not broker_available(), reason="MQTT broker not available - run: docker-compose up -d"
)
...
class TestMQTTCapture:
    def test_can_connect_anonymously(self):
        connected = {"ok": False}
        client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2, client_id="test-anon")
    ...
    def test_wildcard_captures_messages(self):
        ...
        sub = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2, client_id="test-subscriber")
