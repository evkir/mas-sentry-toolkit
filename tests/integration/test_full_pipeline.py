...
class TestFullPipeline:
    ...
    def test_anomaly_detection_on_rogue(
        self, pipeline_fingerprints, pipeline_baseline
    ):
        ...
        assert result[rogue_id].is_rogue or \
               len(result[rogue_id].threat_flags) > 0

    def test_stride_mapping_from_anomalies(
        self, pipeline_fingerprints, pipeline_baseline
    ):
        detector = AnomalyDetector(pipeline_baseline)
        result = detector.analyze(pipeline_fingerprints)
    ...
    def test_report_generation_end_to_end(
        ...
        report.abfp_fingerprints = [
            fp.to_dict() for fp in result.values()
        ]
    ...
    def test_html_report_contains_all_sections(
        ...
        report = MASAuditReport(
            meta=ReportMeta(
                session_id="html-test-001",
                target="127.0.0.1",
                protocol="mqtt"
            )
        )
        report.abfp_fingerprints = [fp.to_dict() for fp in result.values()]
        report.add_finding("Test", "HIGH", "Test finding",
                           remediation="Fix it")
