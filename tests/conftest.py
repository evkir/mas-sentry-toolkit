...
@pytest.fixture
def sample_report():
    report = MASAuditReport(meta=ReportMeta(session_id="fixture-001", target="127.0.0.1", protocol="mqtt"))
    report.add_finding(
        "Anonymous Access",
        "CRITICAL",
...
def clean_fingerprint():
    fp = AgentFingerprint(agent_id="inferred_sensors_sensor_001", first_seen=1000.0, last_seen=1120.0)
    ...
    fp.topic_profiles["sensors/sensor_001/telemetry"] = TopicProfile(
        topic="sensors/sensor_001/telemetry", message_count=120
    )
    return fp
...
def rogue_fingerprint():
    fp = AgentFingerprint(agent_id="inferred_commands_unknown", first_seen=1000.0, last_seen=1045.0)
    fp.timing = TimingMetrics(
    ...
    fp.confidence = 0.9
    fp.topic_profiles["commands/admin/reset"] = TopicProfile(topic="commands/admin/reset", message_count=45)
    return fp
